...
import { Ade } from '@/types/Beverage';
...
const AdeOrderPage = ({ data, target }: InternalOrderPageProps) => {
  const navigate = useNavigate();

  const [amount, setAmount] = useState(0);
  const [current, setCurrent] = useState<Partial<Ade>>({ type: 'ade' });
  ...
  const valid = useMemo(() => {
    ...
    let currentData: Partial<Ade> & { amount?: number } = current;
